...
# model setting
# 数据标准化对训练正确性影响极大!!!!!!!!!
parser.add_argument("--input_dim", type=int, default=feature_num) # 6 features
parser.add_argument("--classes", type=int, default=2) # 2 classes

# dataset setting 修改数据集和Dataset类, 输入类别=======================读入数据
# data = np.genfromtxt('training.data', delimiter=' ')
# 将csv读入为numpy数组
data = np.genfromtxt('training.data', delimiter=' ')
# 将data第六列的-1替换成0, label从-1, 1换成0, 1
data[:, feature_num][data[:, feature_num] == -1] = 0
# data的特征进行标准化
data[:, 0: feature_num] = sklearn.preprocessing.scale(data[:, 0: feature_num])
data = pd.DataFrame(data)
print(data)
train_data, test_data = train_test_split(data, test_size=0.2, random_state=randint(1,100))
...
def main():
    ...
    train_data, test_data = train_test_split(data, test_size=0.3, random_state=randint(1,100))
    train_data = train_data.values   # pandas对象属性
    ...
    # starting training
    max_acc = 0
    max_epoch = 0 # epoch或iteration代表读完所有数据的轮次
...
def evaluate(model, test_loader):
    ...
    report = classification_report(y_true, y_pred, labels=[0, 1], digits=3)
    print(report)

    all_at = np.sum(cm)
    right_at = cm[1][1] + cm[0][0]
    at_accuracy = right_at / all_at
    print('准确率: ', at_accuracy)  
