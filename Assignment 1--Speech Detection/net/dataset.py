...
# from train import feature_num

"""将数据包装成Dataset类, 输入包含特征和标签的numpy数组, 输出Dataset类
...
class DealDataset(Dataset):
    def __init__(self, data: numpy.ndarray):
        ...
        # convert to tensor
        self.x = torch.from_numpy(data[:, 0: 6])  # 前13列是特征
        # self.x = numpy.delete(data, 13, axis=1)
        self.label = torch.from_numpy(data[:, 6])  # 第14列是标签
